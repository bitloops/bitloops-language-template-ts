...
import { IBaseController, ErrorMessage } from '../../IBaseController';

// TODO change all errors to be consistent with ErrorMessage

export abstract class BaseFastifyController
  implements IBaseController<FastifyRequest, FastifyReply>
{
  ...
  public static jsonResponse(res: FastifyReply, code: number, message: string | ErrorMessage) {
  ...
  // }
  public forbidden(res: FastifyReply, message?: ErrorMessage | string) {
    return BaseFastifyController.jsonResponse(res, 403, message ? message : 'Forbidden');
  }

  public notFound(res: FastifyReply, message?: ErrorMessage | string) {
